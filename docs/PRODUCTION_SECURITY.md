...
Este documento separa los controles incluidos en el código de los controles que deben configurarse en Supabase y Cloudflare. Ningún sistema puede prometer riesgo cero; producción requiere mantener ambos grupos.
...
- Worker sin shell, `child_process`, escritura al sistema operativo ni evaluación dinámica. El aislamiento de Cloudflare reduce la superficie de ataques al SO anfitrión.
...
4. Guardar secretos mediante Wrangler/Cloudflare Secrets y rotar cualquier clave que haya pasado por logs, chat o repositorio.
5. En Cloudflare: activar Managed WAF Rules, bot protection y rate limiting. Base recomendada: login/recuperación 5 solicitudes por minuto por IP; APIs mutables 60/min; sincronizaciones y tareas 5/min; webhook Resend 120/min.
6. Restringir `/api/tasks/*` adicionalmente con Cloudflare Access si seguirá disponible manualmente.
...
- La CSP necesita `unsafe-inline` para los scripts de hidratación actuales de Vinext; se debe migrar a nonces cuando el runtime lo soporte de extremo a extremo.
- `vinext` depende de `image-size`, cuyo aviso de DoS para ICNS/JXL/HEIF aún no tiene versión corregida. La aplicación no ofrece carga de esos formatos y la optimización productiva usa Cloudflare Images; revisar y actualizar en cuanto Vinext publique el parche.
- El rate limiting distribuido pertenece a Cloudflare/Supabase y no puede garantizarse solo con código por aislamiento, múltiples regiones y reinicios de Workers.
