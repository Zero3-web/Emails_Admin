...
import { Building2, Check, CheckCircle2, ChevronDown, ChevronLeft, FileSpreadsheet, Filter, Loader2, Mail, Search, ShieldCheck, Tag, Upload, UserMinus, UsersRound, X } from "lucide-react";
...
function FilterMenu({ label, value, options, onChange }: { label: string; value: string; options: FilterOption[]; onChange: (value: string) => void }) {
  const selected = options.find((option) => option.value === value) ?? options[0];
  return (
    <details className="contacts-filter-menu">
      <summary aria-label={label}><span>{selected.label}</span><ChevronDown size={14} /></summary>
      <div role="listbox" aria-label={label}>
        {options.map((option) => (
          <button key={option.value} type="button" role="option" aria-selected={option.value === value} onClick={(event) => { onChange(option.value); event.currentTarget.closest("details")?.removeAttribute("open"); }}>
      ...
      <section className="contacts-workspace">
        ...
        <section className="card contacts-panel">
          ...
          <div className="contacts-toolbar">
            <label className="contacts-search"><Search size={15} aria-hidden="true" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar nombre, correo o empresa" aria-label="Buscar contactos" />{query && <button type="button" onClick={() => setQuery("")} aria-label="Limpiar búsqueda"><X size={13} /></button>}</label>
            <div className="contacts-filter-group"><Filter size={14} aria-hidden="true" /><FilterMenu label="Filtrar por marca" value={siteFilter} options={siteFilterOptions} onChange={setSiteFilter} /><FilterMenu label="Filtrar por interés" value={interestFilter} options={[{ value: "all", label: "Todos los intereses" }, { value: "prime", label: "Oficinas" }, { value: "retail", label: "Locales comerciales" }, { value: "hub", label: "Industrial" }]} onChange={(value) => setInterestFilter(value as "all" | ContactInterest)} /><FilterMenu label="Filtrar por estado" value={statusFilter} options={[{ value: "all", label: "Todos los estados" }, { value: "active", label: "Activos" }, { value: "unsubscribed", label: "Dados de baja" }, { value: "bounced", label: "Rebotados" }, { value: "complained", label: "Quejas" }, { value: "blocked", label: "Bloqueados" }]} onChange={(value) => setStatusFilter(value as "all" | Contact["status"])} /></div>
          ...
          {visibleContacts.length ? (
            ...
            <div className="contacts-empty"><span className="contacts-empty-icon">{hasFilters ? <Search size={21} /> : <UsersRound size={22} />}</span><h3>{hasFilters ? "No encontramos coincidencias" : "Tu audiencia está vacía"}</h3><p>{hasFilters ? "Prueba otra búsqueda o elimina los filtros aplicados." : "Importa un archivo CSV para comenzar a crear segmentos y enviar campañas con consentimiento."}</p>{hasFilters ? <button className="btn" type="button" onClick={clearFilters}>Limpiar filtros</button> : <button className="btn primary" type="button" onClick={() => setImportOpen(true)}><Upload size={15} />Importar primeros contactos</button>}</div>
          )}
        </section>
      </section>

      {importOpen && <div className="contacts-modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && closeImport()}><section className={`card contacts-import ${rows.length ? "has-file" : "file-only"} ${saving ? "is-processing" : ""}`} role="dialog" aria-modal="true" aria-labelledby="contacts-import-title" aria-busy={saving}>
        ...
        <div className="contacts-import-body"><input ref={inputRef} type="file" accept=".csv,text/csv" hidden onChange={(event) => void choose(event.target.files?.[0])} />
          ...
          <div className="contacts-import-workspace"><div className="contacts-import-config"><div className="contacts-file-selected"><FileSpreadsheet size={17} /><span><strong>{fileName}</strong><small>{rows.length.toLocaleString("es-PE")} filas detectadas</small></span><button type="button" onClick={() => { setRows([]); setFileName(""); }} aria-label="Cambiar archivo">Cambiar</button></div>
            <fieldset className="contacts-choice-field"><legend><Building2 size={14} />Marca de destino</legend><div className="contacts-site-options">{sites.map((site) => <button key={site.id} type="button" className={siteId === site.id ? "active" : ""} aria-pressed={siteId === site.id} onClick={() => setSiteId(site.id)}><span style={{ backgroundColor: site.primaryColor }}>{site.name.slice(0, 2).toUpperCase()}</span><strong>{site.name}</strong>{siteId === site.id && <Check size={13} />}</button>)}</div></fieldset>
