  ...
  return (
    <section className="up-card up-email-performance aep-container dashboard-activity">
      <header className="aep-header dashboard-activity-head">
        ...
        <div className="aep-tabs" role="tablist" aria-label="Tipo de actividad">
          <button type="button" role="tab" aria-selected={tab === "emails"} className={`aep-tab ${tab === "emails" ? "active" : ""}`} onClick={() => { setTab("emails"); setQuery(""); }}>Correos <span className="aep-badge">{emails.length}</span></button>
          <button type="button" role="tab" aria-selected={tab === "campaigns"} className={`aep-tab ${tab === "campaigns" ? "active" : ""}`} onClick={() => { setTab("campaigns"); setQuery(""); }}>Campañas <span className="aep-badge">{campaigns.length}</span></button>
        </div>
        <div className="aep-actions">
          <label className="aep-search"><Search size={14} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar" aria-label="Buscar actividad" />{query && <button type="button" onClick={() => setQuery("")} aria-label="Limpiar búsqueda"><X size={13} /></button>}</label>
      ...
      <div className="aep-table-wrap dashboard-activity-content" key={tab}>
        {tab === "emails" ? (
