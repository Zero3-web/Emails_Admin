    ...
    <>
      ...
      <section className="card content-library-panel">
        ...
        <div className="content-toolbar">
          <label className="content-search"><Search size={14} /><input className="search-input" aria-label="Buscar propiedades" value={query} onChange={(event) => resetPage(() => setQuery(event.target.value))} placeholder="Buscar por título, ubicación o código" />{query && <button type="button" aria-label="Limpiar búsqueda" onClick={() => resetPage(() => setQuery(""))}><X size={13} /></button>}</label>
