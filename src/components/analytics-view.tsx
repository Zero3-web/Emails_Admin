...
export function AnalyticsView({ sites, contacts, outboundEmails, usage, initialSiteId = "all" }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return outboundEmails.filter((email) => new Date(email.date).getTime() >= cutoff && (siteId === "all" || email.siteId === siteId));
  ...
  const audience = contacts.filter((contact) => siteId === "all" || contact.siteIds?.includes(siteId));
  const unsubscribed = audience.filter((contact) => contact.status === "unsubscribed").length;
  const currentUsage = siteId === "all" ? usage : usage?.bySite?.[siteId];
  const exportCsv = () => {
