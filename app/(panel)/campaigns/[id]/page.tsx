...
export default async function CampaignDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="campaign-detail-page">
    <PageHeader eyebrow="Campaña" title={campaign.name} description={displaySubject} action={<div className="campaign-header-actions"><Link href="/campaigns"><ArrowLeft size={14} />Volver a campañas</Link><StatusBadge status={campaign.status} /></div>} />
    <div className="campaign-detail-grid"><main>
      <section className="card campaign-overview">
        <div className="campaign-overview-brand"><SiteMark site={site} /><div><span>Marca y remitente</span><strong>{site.name}</strong><small>{site.senderName} · {site.senderEmail}</small></div></div>
        ...
        {!campaign.recipientCount && <div className="campaign-readiness-note"><span><UsersRound size={16} /></span><div><strong>Esta campaña todavía no tiene destinatarios</strong><p>Agrega contactos con consentimiento para habilitar la aprobación.</p></div><Link href="/contacts">Preparar audiencia</Link></div>}
      </section>
      <section className="campaign-frozen"><div className="section-head"><div><h2 className="section-title">Contenido congelado</h2><p className="subtitle">Esta copia no cambiará cuando Tokko o WordPress se actualicen.</p></div><span className="frozen-badge"><LockKeyhole size={13} />Inmutable</span></div><div className="campaign-frozen-grid">
        {items.map((item) => <article className="card campaign-frozen-item" key={item.id}>{item.imageUrl ? <img src={item.imageUrl} alt={item.title} /> : <div className="campaign-frozen-placeholder" />}<div><span>{item.itemType === "property" ? "Oficina" : "Artículo"}</span><h3>{item.title}</h3>{item.itemType === "property" ? <p>{item.location}{item.area ? ` · ${item.area} m²` : ""}</p> : <p>{item.excerpt}</p>}<footer>{item.itemType === "property" && <strong>{item.price ? `${item.currency} ${item.price.toLocaleString("es-PE")}` : "Precio a consultar"}</strong>}<a href={item.publicUrl} target="_blank" rel="noreferrer">Ver publicación <ExternalLink size={13} /></a></footer></div></article>)}
      </div></section>
    </main><aside className="card campaign-actions"><header><span>Estado del flujo</span><strong>{campaign.status === "sent" ? "Campaña enviada" : campaign.status === "scheduled" ? "Envío programado" : campaign.status === "sending" ? "Enviando campaña" : `Siguiente paso: ${campaign.status === "ready" ? "probar o enviar" : campaign.recipientCount ? "aprobar la campaña" : "preparar la audiencia"}`}</strong></header><div className="campaign-stepper" aria-label="Progreso de la campaña">{campaignStages.map((stage, index) => <div className={progress[stage.id] ? "complete" : currentStage === stage.id ? "current" : "pending"} key={stage.id}><span>{progress[stage.id] ? <Check size={12} /> : index + 1}</span><strong>{stage.label}</strong></div>)}</div><CampaignApproval campaign={campaign} canApprove={canApprove} /><Link className="btn" href={`/templates?site=${site.id}`}>Revisar plantilla</Link><small>La prueba y el envío permanecerán bloqueados hasta completar la aprobación.</small></aside></div>
  </div>;
}
